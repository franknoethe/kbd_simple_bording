...
def get_tasks():
    ...
    try:
        ...
        sort_column = sort_columns.get(
            request.args.get("sort", "due_date"), "t.due_date"
        )
        direction = (
            "DESC" if request.args.get("direction", "asc").lower() == "desc" else "ASC"
        )
        try:
...
def task_suggestions():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "suggestions": [dict(row) for row in cursor.fetchall()]}
        )
    except Error as error:
        return jsonify({"success": False, "message": f"Database error: {error}"}), 500
    finally:
...
def assign_tasks():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Mitarbeiter, Eintrittsdatum und Vorlage sind erforderlich.",
                }
            ),
            400,
        )
    ...
    try:
        ...
        if not employee:
            return (
                jsonify({"success": False, "message": "Mitarbeiter nicht gefunden."}),
                404,
            )
        cursor.execute(
            """
        ...
        if not template_tasks:
            return (
                jsonify(
                    {"success": False, "message": "Die Vorlage enthält keine Aufgaben."}
                ),
                400,
            )
        if any(task["responsible_function_id"] is None for task in template_tasks):
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Alle Vorlagenaufgaben benötigen einen Verantwortlichen.",
                    }
                ),
                400,
            )
        ...
        function_ids = list(
            {task["responsible_function_id"] for task in template_tasks}
        )
        ...
        missing_functions = sorted(
            {
                ...
                if task["responsible_function_id"]
        ...
        if missing_functions:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": f"Am Standort des Mitarbeiters fehlt ein Mitarbeiter mit folgender Funktion: {', '.join(missing_functions)}",
                    }
                ),
                400,
            )
...
def create_single_task():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": "Ein Mitarbeiter ist erforderlich."}),
            400,
        )
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Ein gültiges Fälligkeitsdatum ist erforderlich.",
                }
            ),
            400,
        )
    ...
    if not title:
        return (
            jsonify({"success": False, "message": "Ein Titel ist erforderlich."}),
            400,
        )
...
def delete_task(task_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            connection.rollback()
            return (
                jsonify({"success": False, "message": "Aufgabe nicht gefunden."}),
                404,
            )
        connection.commit()
...
def task_email_preview(task_id):
    ...
    try:
        ...
        owner_filter = (
            ""
            if current_user_role().casefold() in {"admin", "manager"}
            else " AND t.employee_id = %s"
        )
        values = [task_id]
        if owner_filter:
        ...
        if task["email_template_id"] is None:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Für diese Aufgabe ist keine E-Mail-Vorlage hinterlegt.",
                    }
                ),
                404,
            )
        ...
        if not current_task:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Aufgabe nicht gefunden oder nicht erlaubt.",
                    }
                ),
                404,
            )
...
def get_template_task_options():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": f"Database error: {str(error)}"}),
            500,
        )
    finally:
        if cursor:
        ...
        return jsonify(
            {
                ...
                "total_pages": total_pages,
                "template_context": (
                    dict(template_context) if template_context else None
                ),
            }
        )
    except Error as error:
...
def create_template_task():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": "Ungültige Werte für Prozessaufgabe."}
            ),
            400,
        )
    except Error as error:
...
def update_template_task(task_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            ...
            return (
                jsonify(
                    {"success": False, "message": "Prozessaufgabe nicht gefunden."}
                ),
                404,
            )
        keys = (
...
def copy_template_tasks(template_id):
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": "Ziel-Template ist erforderlich."}),
            400,
        )
    if target_template_id == template_id:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Quell- und Ziel-Template müssen verschieden sein.",
                }
            ),
            400,
        )
        ...
        if len(cursor.fetchall()) != 2:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Quell- oder Ziel-Template nicht gefunden.",
                    }
                ),
                404,
            )
        ...
        if source_count == 0:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Das Quell-Template enthält keine Prozessaufgaben.",
                    }
                ),
                400,
            )
        ...
        if cursor.fetchone()["total"] != 0:
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Das Ziel-Template enthält bereits Prozessaufgaben.",
                    }
                ),
                409,
            )
...
def get_template_copy_options():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "templates": [dict(item) for item in cursor.fetchall()]}
        )
    except Error as error:
        return (
            jsonify({"success": False, "message": f"Database error: {str(error)}"}),
    ...
    try:
        ...
        if not name or None in (process_type_id, location_id, job_id):
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Prozess, Location, Job und Name sind erforderlich.",
                    }
                ),
                400,
            )
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Ungültige Auswahl für Prozess, Location oder Job.",
                }
            ),
            400,
        )
...
def update_template(template_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            connection.rollback()
            return (
                jsonify({"success": False, "message": "Template nicht gefunden."}),
                404,
            )
        template = {
            "id": template_id,
...
def create_email_template():
    ...
    try:
        ...
        if not name or not subject:
            return (
                jsonify(
                    {"success": False, "message": "Name und Betreff sind erforderlich."}
                ),
                400,
            )
        cursor = connection.cursor(cursor_factory=RealDictCursor)
...
def update_email_template(template_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            connection.rollback()
            return (
                jsonify({"success": False, "message": "Vorlage nicht gefunden."}),
                404,
            )
        template = {
            "id": template_id,
...
def preview_email_template_import():
    ...
    try:
        return jsonify(
            {"success": True, "email_template": parse_oft_file(uploaded_file)}
        )
...
def create_role():
    ...
    try:
        ...
        if not name:
            return (
                jsonify({"success": False, "message": "Name darf nicht leer sein."}),
                400,
            )
...
def get_process_types():
    ...
    try:
        ...
        process_types = cursor.fetchall()
        return jsonify(
            {"success": True, "process_types": [dict(item) for item in process_types]}
        )
    except Error as e:
        return jsonify({"success": False, "message": f"Database error: {str(e)}"}), 500
    finally:
...
def update_process_type(process_type_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            connection.rollback()
            return (
                jsonify({"success": False, "message": "Prozessart nicht gefunden."}),
                404,
            )

        connection.commit()
        return jsonify(
            {"success": True, "process_type": {"id": process_type_id, "name": name}}
        )
    except Error as e:
        connection.rollback()
        return jsonify({"success": False, "message": f"Database error: {str(e)}"}), 500
...
def get_job_options():
    ...
    try:
        ...
        locations = cursor.fetchall()
        return jsonify(
            {"success": True, "locations": [dict(item) for item in locations]}
        )
    except Error as e:
        return jsonify({"success": False, "message": f"Database error: {str(e)}"}), 500
    finally:
        ...
        if not names or location_id in (None, ""):
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Name und Standort sind erforderlich.",
                    }
                ),
                400,
            )
...
def get_functions():
    ...
    try:
        ...
        functions = cursor.fetchall()
        return jsonify(
            {"success": True, "functions": [dict(item) for item in functions]}
        )
    except Error as e:
        return jsonify({"success": False, "message": f"Database error: {str(e)}"}), 500
    finally:
...
def update_function(function_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            connection.rollback()
            return (
                jsonify({"success": False, "message": "Funktion nicht gefunden."}),
                404,
            )
...
def update_location(location_id):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            connection.rollback()
            return (
                jsonify({"success": False, "message": "Standort nicht gefunden."}),
                404,
            )
...
def create_employee():
    ...
    try:
        ...
        return (
            jsonify({"success": True, "employee": employee, "id": employee["id"]}),
            201,
        )
    except Error as e:
        connection.rollback()
