        ...
        switch ($method) {
            case 'GET':
                if ($id > 0) {
                    ...
                    $stmt->execute([':id' => $id]);
                    $role = $stmt->fetch();
                    echo json_encode(['success' => true, 'role' => $role ?: null]);
                    exit;
                }
            ...
            case 'POST':
                ...
                echo json_encode(['success' => true, 'role' => $role, 'id' => $newId]);
                exit;
            case 'PUT':
                if ($id <= 0) {
                ...
                $stmt = $pdo->prepare('UPDATE roles SET role = :role WHERE id = :id');
                $stmt->execute([':id' => $id, ':role' => $input['role'] ?? '']);
